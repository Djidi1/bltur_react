          ...
          <Grid container spacing={0}>
            {columns.map((column) => (
              ...
              >
                ...
                {column.items && column.items.points.map(point => {
                  if (isString(point)) {
                    return <li>{ point }</li>;
                  } else if (point.subTitle) {
                    return <h3 className={classes.columnTitle}>{point.subTitle}</h3>
                  } else if (point.attention) {
                    return (
                      <Alert severity={point.attention.type} className={classes.alert}>
                        <AlertTitle>{point.attention.title}</AlertTitle>
                        {point.attention.text}
                      </Alert>
                      )
                  } else if (point.text) {
                    return <Typography variant='body2'>{point.text}</Typography>
                  }
                  return null;
                }
                )}
